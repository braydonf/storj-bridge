...
BucketsRouter.prototype._getShardHashesByBucketId = function(userId, bucketId, callback) {
  const bucketObjectId = new mongoose.Types.ObjectId(bucketId);

  this.storage.models.Bucket.aggregate([
    {
      $match: {
        user: userId,
        _id: bucketId
    ...
    },
    {
      $project: {
        _id: 1,
        bucketEntryId: "$bucketentries._id",
        frame: "$bucketentries.frame"
      }
    },
    {
      $lookup: {
    ...
    callback(null, result.map((a) => a.shardHash));
  });
};

BucketsRouter.prototype._getShardHashesByBucketEntryId = function(userId, bucketEntryId, callback) {
  const bucketEntryObjectId = new mongoose.Types.ObjectId(bucketEntryId);

  this.storage.models.BucketEntry.aggregate([
    {
      $match: {
        user: userId,
        _id: bucketEntryObjectId
      }
    },
    {
      $project: {
...
BucketsRouter.prototype.destroyBucketById = function(req, res, next) {
  ...
  this._getShardHashesByBucketId(userId, bucketId, (err, hashes) => {
    ...
    Bucket.findOne({
      ...
      // the storage events as ended. If this job does not complete, this
      // action can be safely repeated to complete the task.
      StorageEvent.update({
        user: userId,
        shardHash: { $in: hashes },
        ...
        BucketEntry.remove({ bucket: req.params.id }, (err) => {
          ...
          // As the final step, remove the bucket itself.
          bucket.remove((err) => {
            if (err) {
              return next(new errors.InternalError(err.message));
            }
...
BucketsRouter.prototype._createStorageEvents = function(user, results, callback) {
  const StorageEvent = this.storage.models.StorageEvent;
  const cb = callback ? callback : function(err) {
    if (err) {
      log.warn('_createStorageEvents: failed to save storage event, reason: %s',
               err.message);
    }
  };
  async.eachSeries(results, (item, next) => {
    if (!item.token) {
      // skip events for missing shards
      return next();
    }
    const storageEvent = new StorageEvent({
      ...
      user: user._id,
      client: user._id,
      farmer: item.farmer.nodeID,
      timestamp: Date.now(),
      shardHash: item.hash,
      downloadBandwidth: item.size,
      storage: 0,
      success: false
    });
    storageEvent.save(next);
  }, cb);
};
    ...
    User.findOne({
      ...
      BucketEntry.findOne({
        ...
        self._getPointersFromEntry(entry, {
          ...
          // Create storage events for each pointer/shard
          self._createStorageEvents(user, results);
...
BucketsRouter.prototype.removeFile = function(req, res, next) {
  ...
  Bucket.findOne({
    ...
    BucketEntry.findOne({
      ...
      this._getShardHashesByBucketEntryId(userId, bucketEntryId, (err, hashes) => {
        if (err) {
          return next(new errors.InternalError(err.message));
        }
