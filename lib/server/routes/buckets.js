...
BucketsRouter.prototype.destroyBucketById = function(req, res, next) {
  ...
  this._getShardHashesByBucketId(userId, bucketId, (err, hashes) => {
    ...
    Bucket.findOne({
      ...
      // the storage events as ended, as if this job does not complete.
      // This action can be saftely repeated to complete the task.
      StorageEvent.update({
        user: userId,
        shardHash: { $in: hashes },
        storeEnd : { $exists: false }
      }, {
        $currentDate: {
          storeEnd: true
        }
...
BucketsRouter.prototype._createStorageEvents = function(entry, user, results) {
  async.eachSeries(results, (item, next) => {
    ...
    const storageEvent = new StorageEvent({
      ...
      frame: entry.frame,
      user: user._id,
      farmer: item.farmer.nodeID : null,
      timestamp: Date.now(),
      shardHash: item.hash,
    ...
    User.findOne({
      ...
      BucketEntry.findOne({
        ...
        self._getPointersFromEntry(entry, {
          ...
          self._createStorageEvents(entry, user, results);
