...
ReportsRouter.prototype.createExchangeReport = function(req, res, next) {
  ...
  this.storage.models.StorageEvent.findOne({token: token}, (err, event) => {
    ...
    // TODO handle case for mirrors where user is a farmer
    if (req.user && !event.userReport) {
      ...
      event.userReport = {
        exchangeStart: null,
        exchangeEnd: null,
        exchangeResultCode: null,
        exchangeResultMessage: null
      }

      // Give reputation points and mark exchange as successful
    ...
    } else if (req.farmerNodeID && !event.farmerReport) {
      if (event.farmer !== req.farmerNodeID) {
        return next(new errors.NotAuthorized());
      }
      modified = true;
      event.farmerReport = {
