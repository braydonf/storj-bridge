...
ReportsRouter.prototype.createExchangeReport = function(req, res, next) {
  ...
  this.storage.models.StorageEvent.findOne({token: token}, (err, event) => {
    ...
    const isUserReport = (event.user === req.user.id ||
                          event.user === req.farmerNodeID);
    const isFarmerReport = (event.farmer === req.farmerNodeID);

    if (!isUserReport && !isFarmerReport) {
      return next(new errors.NotAuthorized('Not authorized to report'));
    }

    let modified = false;

    if (isUserReport && !event.userReport) {
      modified = true;
      event.userReport = report;
...
ReportsRouter.prototype._createStorageEvent = function(token, event, farmer) {
  ...
  // TODO
  // - consider keeping track of original user
  const storageEvent = new StorageEvent({
    token: token,
    frame: event.frame, // use frame from original upload event
    user: event.farmer, // farmer that already has the mirror
    farmer: farmer.nodeID, // farmer storing the new mirror
    timestamp: Date.now(),
    downloadBandwidth: 0,
    storage: 0,
    farmerStorage: event.storage, // use size from original upload event
    shardHash, event.shardHash, // use hash from original upload event
    success: false
  });
