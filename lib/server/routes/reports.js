...
ReportsRouter.prototype.createExchangeReport = function(req, res, next) {
  ...
  // MODIFY STORAGE EVENT
  // - lookup storage event by token (id)
  //   - if it's not found it's an error, bail
  // - authenticate reporter (do in middleware or here)
  //   - if not auth'd, bail
  // - modify storage event to have
  //   - senderReport/receiverReport
  //     - exchangeStart
  //     - exchangeEnd
  //     - exchangeResultCode
  //     - exchangeResultMessage
  // - update user with fail/success rate
  // - update storage event as successful on receiver success report
  // - update farmer contact with success/fail
...
ReportsRouter.prototype._triggerMirrorEstablish = function(n, hash, done) {
  ...
  function _getRetrievalTokenFromFarmer(mirror, callback) {
    ...
    async.until(test, (done) => {
      ...
      // CREATE STORAGE EVENT HERE
      // token (id) - unique identifier for this event
      // bucket (id)
      // bucketEntry (id)
      // client (user-id)
      // farmer (farmer-id)
      // timestamp (number)
      // downloadBandwidth (number) (nothing)
      // storage (number) (nothing)
      // mirror (number) (add to this)
      // shardHash (id)
