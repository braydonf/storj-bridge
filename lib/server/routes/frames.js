...
FramesRouter.prototype._createStorageEvent = function(frame, user, farmer, pointer) {
  const storageEvent = new StorageEvent({
    ...
    farmerStorage: pointer.size,
    timestamp: Date.now(),
    shardHash: pointer.hash,
    success: false,
  });

  storageEvent.save(function(err) {
    if (err) {
      ...
      Pointer.create(pointerData, function(err, pointer) {
        ...
        Frame.findOne({
          ...
          frame.addShard(pointer, (err) => {
            ...
            // Keep track of this storage event
            self._createStorageEvent(frame, req.user, farmer, pointer);
